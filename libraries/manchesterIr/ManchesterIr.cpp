...
ManchesterIr::ManchesterIr() //constructor
{
  applyWorkAround1Mhz = 0;
}


void ManchesterIr::setTxPin(uint8_t pin)
{
...
void ManchesterIr::setRxPin(uint8_t pin)
{
  ::RxPin = pin; // user sets the digital pin as output
  pinMode(::RxPin, INPUT); 
}

void ManchesterIr::workAround1MhzTinyCore(uint8_t a)
{
  applyWorkAround1Mhz = a;
}

void ManchesterIr::setupTransmit(uint8_t pin, uint8_t SF)
{
  setTxPin(pin);
  speedFactor = SF;
...
void ManchesterIr::setupReceive(uint8_t pin, uint8_t SF)
{
  setRxPin(pin);
...
void ManchesterIr::setup(uint8_t Tpin, uint8_t Rpin, uint8_t SF)
{
...
void ManchesterIr::transmit(uint16_t data)
{
...
*/
void ManchesterIr::transmitArray(uint8_t numBytes, uint8_t *data)
{
...
void ManchesterIr::sendZero(void)
{
...
void ManchesterIr::sendOne(void)
{
  delayMicroseconds(delay1);
  //digitalWrite(TxPin, LOW);
  //TIMER_DISABLE_PWM;
...
uint8_t ManchesterIr::decodeMessage(uint16_t m, uint8_t &id, uint8_t &data)
{
  //extract components
  data = (m & 0xFF);
  id = (m >> 12);
...
uint16_t ManchesterIr::encodeMessage(uint8_t id, uint8_t data)
{
...
void ManchesterIr::beginReceiveArray(uint8_t maxBytes, uint8_t *data)
{
  ::MANRX_BeginReceiveBytes(maxBytes, data);
}

void ManchesterIr::beginReceive(void)
{
  ::MANRX_BeginReceive();
}


uint8_t ManchesterIr::receiveComplete(void)
{
  return ::MANRX_ReceiveComplete();
}


uint16_t ManchesterIr::getMessage(void)
{
  return ::MANRX_GetMessage();
}


void ManchesterIr::stopReceive(void)
{
  ::MANRX_StopReceive();
}
...
void MANRX_SetupReceive(uint8_t speedFactor)
{
  ...
  //setup timers depending on the microcontroller used
    ...
    */
    ...
    #else
    #error "ManchesterIr library only supports 1mhz, 8mhz, 16mhz, 16.5Mhz clock speeds on ATtiny85 chip"
    #endif
    ...
    */
      ...
      OCR1A = (64 >> speedFactor) - 1; 
    #else
    #error "ManchesterIr library only supports 1mhz, 8mhz clock speeds on ATtiny2313 chip"
    #endif
    ...
    */
    ...
    #else
    #error "ManchesterIr library only supports 1mhz, 8mhz, 16mhz on ATtiny84"
    #endif
    ...
    */
    ...
    #else
    #error "ManchesterIr library only supports 1mhz, 8mhz, 16mhz on ATMega32U4"
    #endif
    ...
    */
    ...
    #else
    #error "ManchesterIr library only supports 1Mhz, 8mhz, 16mhz on ATMega8"
    #endif
    ...
    */
    ...
    TCCR2A = _BV(WGM21); // reset counter on match
      ...
      OCR2A = (128 >> speedFactor) - 1; 
    #else
    #error "ManchesterIr library only supports 8mhz, 16mhz on ATMega328"
    #endif
...
void ManchesterIr::enableIROut (int khz)
{
